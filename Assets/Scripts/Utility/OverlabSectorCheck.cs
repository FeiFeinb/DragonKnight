...
namespace RPG.Utility
{
    public class OverlabSectorCheck : BaseOverlabCheck
    {
        ...
        private void Update()
        {
            ...
            int resultsValue = Physics.OverlapSphereNonAlloc(checkVec, radius, _results, _collideLayer.value);
            if (resultsValue <= 0)
            {
                _isCollide = false;
                return;
            ...
            _isCollide = Mathf.Abs(Vector3.Angle(_results[0].transform.position - transform.position, transform.forward)) < angle;
        }
