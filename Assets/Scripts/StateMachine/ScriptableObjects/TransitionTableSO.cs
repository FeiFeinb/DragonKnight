...
namespace RPG.StateMachine
{
    ...
    public class TransitionTableSO : ScriptableObject
    {
        ...
        public State globalState => cachedGlobalState;

        [SerializeField] private TransitionItem[] transitionItems; // 状态及其转换
        [SerializeField] private StateSO globalStateSO;

        [SerializeField] private List<State> cachedState; // 缓存初始化完毕的状态数列
        [SerializeField] private State cachedGlobalState; // 缓存的全局状态
        ...
        public void InitTransitionTable(StateMachine stateMachine)
        {
            ...
            foreach (var fromStateGroup in fromStateGroups)
            {
                ...
                fromState.SetStateTransition(GetStateTransition(stateMachine, createInstance,
                    fromStateGroup.ToArray()));
                // 添加至缓存数列
                cachedState.Add(fromState);
            }

            if (!globalStateSO) return;
            cachedGlobalState = globalStateSO.GetState(stateMachine, createInstance);
            cachedGlobalState.SetStateAction(globalStateSO.GetActions(stateMachine, createInstance));
        }

        private StateTransition[] GetStateTransition(StateMachine stateMachine,
            Dictionary<ScriptableObject, object> createInstance, TransitionItem[] transitionItems)
        {
            StateCondition[] ProgressCondition(StateMachine stateMachine,
                Dictionary<ScriptableObject, object> createInstance, ConditionPair[] conditionPairs)
            {
                ...
                {
                    stateConditions.Add(conditionPair.condition.GetStateCondition(stateMachine, createInstance,
                        conditionPair.expectedResult));
                }
            ...
            foreach (TransitionItem transitionItem in transitionItems)
            {
                ...
                transitions.Add(new StateTransition(toState,
                    ProgressCondition(stateMachine, createInstance, transitionItem.conditions)));
            }
